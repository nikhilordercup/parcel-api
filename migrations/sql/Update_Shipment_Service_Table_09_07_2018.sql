ALTER TABLE `icargo_shipment_service` ADD `label_tracking_number` INT NOT NULL AFTER `status`, ADD `label_files_png` VARCHAR(255) DEFAULT NULL AFTER `label_tracking_number`, ADD `label_file_pdf` VARCHAR(255) NOT NULL AFTER `label_files_png`, ADD `is_label_printed` VARCHAR(1) NOT NULL COMMENT 'Y for yes,N for No'AFTER `label_file_pdf`; 
