...
$app->post('/signUp', function() use ($app) {
    ...
    if(!$isUserExists){
        ...
        $table_name = "icargo_users";
        
        $column_names = array('name','contact_name','phone','email','password','address_1','address_2','city','postcode','user_level','uid','register_in_firebase','state','country');
...
$app->post('/listAllPlanForCustomerRegistration', function() use ($app){
	$db = new DbHandler();
	$response = $db->getAllRecords("SELECT plan_id,plan_name FROM " . DB_PREFIX ."chargebee_plan WHERE status='active'");
	echoResponse(200, $response);
});
?>