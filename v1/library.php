...
class Library
{
    ...
    public $google_api_key = "AIzaSyBhIrfuaPZmIGXV8KO5jCSE-Tpkr8J-7Z8";

    public $google_map_url = "https://maps.googleapis.com/maps/api";
    ...
    function get_file_content($param)
    {
        $content = null;
    ...
    function put_file_content()
    {
    }

    public

    function generateCheckDigit($base_val)
    {
        $result = "";
        $weight = array(
            2, 3, 4,
    ...
    function get_address_by_postcode($postcode, $lat = false, $long = false)
    { //recommended by nikhil sir for same day booking
    ...
    function get_lat_long_by_address($address)
    {
        ...
        $api_url = "$this->google_map_url/geocode/json?address=$address&sensor=false";
    ...
    function get_lat_long_by_address_for_resolve_route($address)
    {
        $address = urlencode($address);
        $api_url = "$this->google_map_url/geocode/json?address=$address&key=$this->google_api_key";
        $query = $this->get_curlresponse($api_url);
        if ($query != 'Error') {
            $json = json_decode($query);
            if ($json->status == "OK") {
    ...
    function get_lat_long_by_postcode($address)
    {
        $address = urlencode($address);
        $api_url = "$this->google_map_url/geocode/json?address=$address&key=$this->google_api_key";
    ...
    function multiple_destinations_distance_and_duration($param)
    {
        ...
        $api_url = "$this->google_map_url/distancematrix/json?units=imperial&origins=$origin&destinations=$destinations&departure_time=$departure_time&traffic_model=best_guess&mode=$mode&key=$key";
        try {
            $curl_handle = curl_init();
            ...
            elseif ($json->status == "INVALID_REQUEST") {
                ...
                return array(
                    "status" => "error",
                    "message" => "Api returns bad response",
                    "json" => $json
                );
            }
        }
    ...
    function get_date_time_format($time)
    {
    ...
    function base_url()
    {
        return sprintf("%s://%s%s", isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] != 'off' ? 'https' : 'http', $_SERVER['SERVER_NAME'], substr($_SERVER['REQUEST_URI'], 0, strpos($_SERVER['REQUEST_URI'], "/v1")));
    }
    ...
    function get_tracking_url_by_env()
    {
    ...
    function get_address_by_latlong($lat, $long)
    {
        $api_url = "$this->google_map_url/geocode/json?latlng=$lat,$long=true";
        $query = $this->get_curlresponse($api_url);
        if ($query != 'Error') {
            $json = json_decode($query);
            if ($json->status == "OK") {
    ...
    function saveImage($file_path, $file_name, $encoded_string)
    {
