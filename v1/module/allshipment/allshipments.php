   ...
   public function getDropTrackingDetails($identity){
	...
	public function printLabelByLoadIdentity($param){
			...
			if(count($labelInfo)==1){
				if($labelInfo[0]['label_file_pdf']!==''){
					if($labelInfo[0]['invoice_created']==1){
						...
						return array("status"=>"success","file_path"=>$labelInfo[0]['label_file_pdf'],"message"=>"1");
					}
			...
			elseif(count($labelInfo)>1){
				...
				return array("status"=>$label_pdf['status'],"file_path"=>$label_pdf['file_path'],"message"=>"2");
