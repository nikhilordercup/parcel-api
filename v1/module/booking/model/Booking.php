...
class Booking_Model_Booking
{
    ...
    function getCustomerCarrierServices($customer_id, $carrier_id, $account_number){
        ...
        $sql = "SELECT CST.service_code, CST.service_name FROM " . DB_PREFIX . "company_vs_customer_vs_services AS CCST INNER JOIN " . DB_PREFIX . "courier_vs_services_vs_company AS CSCT ON CCST.company_service_id = CSCT.id  INNER JOIN " . DB_PREFIX . "courier_vs_services AS CST ON CST.id = CSCT.service_id WHERE CSCT.status= 1 AND CCST.status= 1 AND CST.status = 1 AND CCST.courier_id= '$carrier_id' AND CCST.company_customer_id='$customer_id'";
