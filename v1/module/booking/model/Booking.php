...
class Booking_Model_Booking
{
    ...
    public function getCustomerCarrierAccount($customer_id)
    {
        ...
        $sql = "SELECT CT.id AS carrier_id, CT.name, CT.code AS carrier_code, CCT.account_number, CCT.username AS username, CCT.password AS password FROM ". DB_PREFIX ."courier_vs_company_vs_customer AS CCCT INNER JOIN ". DB_PREFIX ."courier_vs_company AS CCT ON CCT.company_id = CCCT.company_id AND CCT.courier_id = CCCT.courier_id INNER JOIN ". DB_PREFIX ."courier AS CT ON CCCT.courier_id = CT.id WHERE CCCT.customer_id = '$customer_id' AND CCCT.status = 1 AND CT.status=1;";
        return $this->_db->getAllRecords($sql);
    }

    public function getCustomerCarrierServices($customer_id, $carrier_id){
        //$sql = "SELECT CST.service_code, CST.service_name FROM " . DB_PREFIX . "courier_vs_services AS CST INNER JOIN " . DB_PREFIX . "courier_vs_services_vs_company AS CSCT ON CST.id = CSCT.service_id INNER JOIN " . DB_PREFIX . "company_vs_customer_vs_services AS CCST ON CCST.id = CSCT.service_id WHERE CSCT.status=1 AND CCST.company_customer_id='$customer_id' AND CCST.status=1 AND CST.status = 1 AND CCST.courier_id='$carrier_id'";
    ...
    public function getCarrierInfo($customer_id){
    ...
    public function checkCustomerAccountStatus($customer_id){
        $sql = "SELECT status FROM ".DB_PREFIX."users WHERE id = '$customer_id' AND status=1";
        return $this->_db->getRowRecord($sql);
    }

    public function getAddressBySearchStringAndCustomerId($customer_id, $search_string){
    ...
    private function _testShipmentTicket($shipment_ticket){
    ...
    private function _testParcelTicket($parcel_ticket){
    ...
    public function generateTicketNo($company_id){
    ...
    public function generateParcelTicketNumber($company_id){
