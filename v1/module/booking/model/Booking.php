...
class Booking_Model_Booking
{
    ...
    function getCustomerCarrierServices($customer_id, $carrier_id, $account_number, $flowType=""){
        ...
        $flowTypeJoin = ($flowType) ? " INNER JOIN `".DB_PREFIX."service_flow_type` AS CSFT ON CST.id=CSFT.service_id" : '';
        $flowTypeCond = ($flowType) ? " AND LOWER(CSFT.flow_type)='$flowType' " : '';
        $sql = "SELECT CST.service_code, CST.service_name, CSFT.service_id FROM `".DB_PREFIX."courier_vs_services_vs_company` AS CSCT"
                . " INNER JOIN `".DB_PREFIX."company_vs_customer_vs_services` AS CCST ON CSCT.service_id=CCST.service_id"
                . " INNER JOIN `".DB_PREFIX."courier_vs_services` AS CST ON CST.id=CSCT.service_id"
                . " INNER JOIN `".DB_PREFIX."courier_vs_company` AS CCT ON CCT.company_id=CCST.company_id AND CCT.account_number='$account_number'"
                . $flowTypeJoin
