...
class Controller extends Icargo{
    ...
    function _getCustomerByCompanyId($company_id, $warehouse_id){
        ...
        //echo $sql;die;
        $sql = "SELECT UT.id as id, UT.name as name, email as email FROM " . DB_PREFIX . "users AS UT INNER JOIN " . DB_PREFIX . "company_users AS CUT  ON CUT.user_id=UT.id WHERE CUT.company_id  = '$company_id' AND CUT.warehouse_id='$warehouse_id' AND UT.user_level  = '5' AND UT.status=1 ORDER BY name";
        return $this->_parentObj->db->getAllRecords($sql);
    }
