 ...
 public function getCustomerSurchargeDetails($data){ 
  ...
  /*public function getUserDataByCustomerId($customerId){
	...
	public function addAddress($param){
			...
			if($isValidPostcode){
				...
				if($latLngArr['latitude']!='' || $latLngArr['longitude']!=''){
					...
					//$insertData = array("phone"=>$param->phone,"name"=>$param->name,"email"=>$param->user_email,"postcode"=>$param->postcode,"address_line1"=>$param->address_1,"address_line2"=>$param->address_2,"city"=>$param->city,"state"=>$param->state,"country"=>$param->country,"latitude"=>$latLngArr['latitude'],"longitude"=>$latLngArr['longitude'],"customer_id"=>$param->customer_id,"address_type"=>$param->address_type,"search_string"=>$searchString);
					//$column_names = array('phone', 'name', 'email', 'postcode','address_line1','address_line2','city','state','country','latitude','longitude','customer_id','address_type','search_string');

                    $insertData = array("phone"=>$param->phone,"name"=>$param->name,"email"=>$param->user_email,"postcode"=>$param->postcode,"address_line1"=>$param->address_1,"address_line2"=>$param->address_2,"city"=>$param->city,"state"=>$param->state,"country"=>$param->country->short_name,"iso_code"=>$param->country->alpha3_code,"latitude"=>$latLngArr['latitude'],"longitude"=>$latLngArr['longitude'],"customer_id"=>$param->customer_id,"address_type"=>$param->address_type,"search_string"=>$searchString);
                    $column_names = array('phone', 'name', 'email', 'postcode','address_line1','address_line2','city','state','country','iso_code','latitude','longitude','customer_id','address_type','search_string');
	...
	public function searchAddressByUserId($param){	
	    return $this->db->getRowRecord("SELECT  ABT.* FROM  ".DB_PREFIX."address_book ABT INNER JOIN ".DB_PREFIX."user_address AS UAT ON ABT.id=UAT.address_id WHERE UAT.user_id='".$param->user_id."'");
	}
