...
class Package_Model_Index
{
    ...
    function getParcelPackageByUserId($user_id){
        return $this->_db->getAllRecords("SELECT package_code AS package_code, type AS name, weight AS weight, length AS length, height AS height, width AS width,contents AS package_content FROM " . DB_PREFIX ."package_type AS PT INNER JOIN " . DB_PREFIX ."package_type_allowed_users AS PAUT ON PT.id=PAUT.package_id WHERE PAUT.user_id='$user_id'");
    }
