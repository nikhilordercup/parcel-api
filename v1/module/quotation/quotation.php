<?php
    class Quotation extends Library
    {
		...
		private function _saveSamedayQuote($param){
			$shipmentQuoteNumber = $this->_save_quote($param);
			if($shipmentQuoteNumber){
				return array("status"=>"success", "quote_number"=>$shipmentQuoteNumber);
			}else{
				return array("status"=>"error","message"=>"error while saving quote, please try again");
			}
		}

		private function _save_quote($param){
		...
		private function _saveQuoteService($param){
			$service_id = $this->db->save("quote_service", $param);
			return $service_id;
		}
		
		private function _saveQuote($data){
				...
				foreach($data->delivery_shipment_address as $delivery_data) {
					$shipmentData = array("shipment_postcode"=>$delivery_data->postcode,
				...
				if($shipmentStatus['status']=='success'){
					...
					if($serviceId){
						...
						$response = array("status"=>"success","quote_number"=>$shipmentStatus["quote_number"],"message"=>"Quotation saved successfully. Quote Number -".$shipmentStatus["quote_number"]);
					}else{
						$response = array("status"=>"error","quote_number"=>"","message"=>"Quotation not saved, please try again.");
					}
				}	
		...
		public function saveQuoteForCustomer($data){
			...
			if($quoteSave['status']=="success")
				return array("status"=>"success","message"=>"quote saved successfully.Quote Number - ".$quoteSave['quote_number']." ");
			else
				return array("status"=>"error","message"=>"quote not saved,please try again");
			
		}

		private function _getCustomerQuoteExpiryDate($customer_id,$service_date){
		...
		private function _getShipmentDataByQuoteNumber($quote_number){
		...
		private function _getCourierDataByCompanyId($company_id){
			return $this->db->getRowRecord("SELECT name,icon FROM `".DB_PREFIX."courier` AS CT WHERE `company_id`='$company_id'");
		}
		
		private function _getCustomerQuoteExpiryDays($customer_id){
			$record = $this->db->getRowRecord("SELECT quote_expiry_days FROM " . DB_PREFIX . "customer_info WHERE `user_id` = '$customer_id'");
			return $record['quote_expiry_days'];
		}
		

		private function _test_shipment_quote($quote_number){
		...
		public function sendQuoteEmail($data){
			...
			if($data->quote_email!=''){
				...
				if($quoteSave['status']=="success"){
					...
					$response = array("status"=>"error","message"=>$quoteSave['message']);
				}
		...
		public function getAllSavedQuotes($param){
		...
		public function getAllSavedQuotesByCompanyId($param){
			$quoteArr = array();
			...
			foreach($quoteData as $value){
				...
				$quoteArr[$value['quote_number']] = array("quote_number"=>$value['quote_number'],"postcode"=>$value['postcode'],"name"=>$value['name'],"email"=>$value['email_id'],"service"=>$serviceArr,"expiry_date"=>$value['expiry_date']);
			}			
			
			return $quoteArr;
		}
		
		public function getQuoteDataByQuoteNumber($param){
			$quoteArr = array();
			...
			$quoteArr['customer'] = array("id"=>$customerData['id'],"name"=>$customerData['name'],"email"=>$customerData['email']);
			...
			$quoteArr['user'] = array("id"=>$userData['id'],"name"=>$userData['name']);
