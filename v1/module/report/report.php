...
class Report extends Icargo
    ...
    private function _findSamedayRevenue($load_identity_str)
        {
        $item = $this->modelObj->findSamedayRevenue($load_identity_str);
        $allDropByLoadIdentity = $this->modelObj->findAllDropByLoadIdentity($load_identity_str);
        ...
        if(isset($item["carrier_price"])){
            $allDropCount = count($allDropByLoadIdentity);
            $pricePerDrop = $item["carrier_price"]/$allDropCount;
            $revenuePrice["driver_carrier_price_per_drop"] = $pricePerDrop;
        }

        if(isset($item["customer_price"])){
            $allDropCount = count($allDropByLoadIdentity);
            $pricePerDrop = $item["customer_price"]/$allDropCount;
            $revenuePrice["driver_customer_price_per_drop"] = $pricePerDrop;
        }
        return $revenuePrice;
        }

    private function _findRevenuePrice($job_count, $drop_count, $load_identity_str)
        {
        $revenuePrice = 0;
        if($this->_getServiceTypeName()=="SAME"){
            $revenuePrice = $this->_findSamedayRevenue($load_identity_str);
            $revenuePrice = array(
                "driver_carrier_price_per_drop" => $revenuePrice["driver_carrier_price_per_drop"]  * $drop_count,
                "driver_customer_price_per_drop" => $revenuePrice["driver_customer_price_per_drop"]  * $drop_count
            );
        }
        else{
            $revenuePrice = $this->revenuePricePerJob * $job_count;
            $revenuePrice = array(
                "driver_carrier_price_per_drop" => $revenuePrice,
                "driver_customer_price_per_drop" => $revenuePrice
            );
        }
        return $revenuePrice;
        }
    ...
    private function _generateDerReport()
        ...
        foreach($driverTimeInfo as $driver_id => $item){
            ...
            $reportData[$driver_id]["no_of_drops"] = $item["total_drop_count"];
            ...
            $price = $this->_findRevenuePrice($totalJobCount, $totalDropCount, $load_identity_str);

            $reportData[$driver_id]['revenue'] = round($price["driver_carrier_price_per_drop"], 2);
            $reportData[$driver_id]['revenue1'] = round($price["driver_customer_price_per_drop"], 2);
