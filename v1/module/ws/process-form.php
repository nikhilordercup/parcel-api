...
class Process_Form{
 ...
 private function _add_shipment_parcel_life_cycle($ticket, $parcelticket, $assigned_driver, $shipment_routed_id, $lattitude, $longitude, $msg, $isScan = false,$actionCode,$loadIdentity='',$data=array()) 
    {
        $parceldetails = $this->model_rest->get_parcel_details_by_parcel_ticket($parcelticket);   
		$shipmentHistoryData                    = array();
        $shipmentHistoryData['shipment_ticket'] = $ticket;
        $shipmentHistoryData['parcel_ticket']   = ($parcelticket!='')?$parcelticket:'';
		$shipmentHistoryData['instaDispatch_pieceIdentity']   = ($parcelticket!='')?$parceldetails['instaDispatch_pieceIdentity']:'';
		$shipmentHistoryData['instaDispatch_loadIdentity']   = ($parcelticket!='')?$parceldetails['instaDispatch_loadIdentity']:$loadIdentity;
        $shipmentHistoryData['create_date']     = date("Y-m-d");
        $shipmentHistoryData['create_time']     = date("H:m:s");
        $shipmentHistoryData['actions']         = $msg;
		$shipmentHistoryData['internel_action_code'] = $actionCode;
        $shipmentHistoryData['driver_id']       = ($assigned_driver == '') ? 0 : $assigned_driver;
        $shipmentHistoryData['is_scaning']      = ($isScan) ? 'YES' : 'NO';
        $shipmentHistoryData['route_id']        = ($shipment_routed_id == '') ? 0 : $shipment_routed_id;
        $shipmentHistoryData['taken_action_by'] = (($lattitude == '') && ($longitude == '')) ? 'controller' : 'Driver';
        $shipmentHistoryData['lattitude']       = $lattitude;
        $shipmentHistoryData['longitude']       = $longitude;
        $shipmentHistoryData['warehouse_id']    = $this->warehouse_id;
        $shipmentHistoryData['company_id']      = $this->company_id;
		$shipmentHistoryData['data']      		= json_encode($data);
        $statusReturn                           = $this->model_rest->save('shipment_life_history', $shipmentHistoryData);
        return $statusReturn;
    }
    
    private function _add_shipment_life_cycle($ticket, $assigned_driver, $shipment_routed_id, $msg, $actionCode, $lattitude='', $longitude='', $isScan = false,$data=array())
    {   
        $allShipParceldetails = $this->model_rest->get_shipment_parcel_status_details($ticket);
		
        if (count($allShipParceldetails) > 0) 
        {
            foreach ($allShipParceldetails as $shipdetails)
            {
                $this->_add_shipment_parcel_life_cycle($ticket, $shipdetails['parcel_ticket'], $assigned_driver, $shipment_routed_id, $lattitude, $longitude, $msg, $isScan, $actionCode);
            }
        }
        else
        {
		    $getshipmentDetails = $this->model_rest->get_only_shipment_details_by_ticket($ticket);
			if($getshipmentDetails['instaDispatch_loadGroupTypeCode'] == 'SAME')
            {
			    $this->_add_shipment_parcel_life_cycle($ticket, '', $assigned_driver, $shipment_routed_id, $lattitude, $longitude, $msg, $isScan, $actionCode,$getshipmentDetails['instaDispatch_loadIdentity'],$data);
			}
		}
        return true;
    }
    ...
    public function process()
    {
        ...
        else if($this->loadActionCode == 'processdriverfailaction')
        {
            ...
            if($data['status']=="success")
            {
                ...
                Consignee_Notification::_getInstance()->sendShipmentCollectionDeliverNotification(array("service_message"=>$this->service_message,"shipment_ticket"=>$this->shipment_ticket,"company_id"=>$this->company_id,"warehouse_id"=>$this->warehouse_id,"trigger_code"=>"failed"));
            }
        }    
        return $data;
    }
